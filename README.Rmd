...
analyzing groundwater data that is in a [tidy format](https://cran.r-project.org/web/packages/tidyr/vignettes/tidy-data.html). Manager also provides 
...
```
```{r, fig.height=6, fig.width=9, results='hide', cache=TRUE}
...
Create Piper plots...
```{r, fig.height=6, fig.width=9, results='hide', cache=TRUE, warning=FALSE}
gw_data %>%
  select_all(., toupper) %>%
  piper_plot()
```
